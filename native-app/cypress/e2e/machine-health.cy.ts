describe("Welding Robot Machine Health", () => {
  it("Calculates Machine Health and resets data", () => {
    ...
    cy.get(".css-text-146c3p1").should("contain", 87.5);
    cy.get("button").contains("Reset Machine Data").click();
    cy.contains("Please log a part to check machine health").should(
      "be.visible"
    );
  });
});
...
describe("Multiple Machine Health", () => {
  it(
    "Calculates Machine Health and resets data for Painting Station, Assembly Line and Quality Control Station",
    { defaultCommandTimeout: 1100 },
    () => {
      cy.visit("/");
      ...
      btn.click();
      cy.get('[data-testid="web_picker"]')
        .first()
        .select("qualityControlStation");
